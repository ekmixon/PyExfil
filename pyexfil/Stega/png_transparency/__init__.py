...
class Encode(object):
	...
	def Run(self):
		if not self.generate_empty_image:
			...
			pxlsArray = [
				[255, 255, 255, 0]
				for _ in range(0, len(self.encrypted_data) + 1 + len(TERMINATOR))
			]
		finalArray = [
			(
				pxlsArray[index][0],
		...
		finalArray.extend(
			...
			for i, index in enumerate(
				range(
					len(self.encrypted_array),
					len(self.encrypted_array) + len(TERMINATOR),
				)
			)
		)
...
class Decode():
	...
	def Run(self):
		...
		search_array = [pixel[3] for pixel in pxlsArray]
